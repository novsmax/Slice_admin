...
const ProductImagesPage = () => {
    ...
    const handleAddImage = async () => {
        try {
            ...
            // Устанавливаем product_id выбранного товара
            const imageData = {
                ...formValues,
                product_id: selectedProduct.id
            };

            await productImageService.createProductImage(imageData);
            handleCloseDialogs();
            fetchProductImages(selectedProduct.id);
        } catch (err) {
            console.error('Error adding image:', err);
            setError('Не удалось добавить изображение');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Box>
            ...
            <Dialog open={openAddDialog} onClose={handleCloseDialogs} maxWidth="sm" fullWidth>
                <DialogTitle>Добавить новое изображение</DialogTitle>
                <DialogContent>
